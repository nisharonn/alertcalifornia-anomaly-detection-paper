...
To ensure robust evaluation, we implement a stratified 5-fold cross-validation framework. The dataset is partitioned into five folds using stratified sampling based on class labels and camera location so that each fold has a representative distribution of anomaly severity and geographic location. During each iteration, four folds are used for training, while the final fold is held out as the validation set. 
