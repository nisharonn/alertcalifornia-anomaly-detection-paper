...
To ensure robust evaluation, we implement a stratified 4-fold cross-validation framework. The training set is partitioned into four folds using random stratified sampling based on class labels so that each fold has a representative distribution of anomaly severity. During each iteration, three folds are used for training, while the final fold is held out as the validation set. 
...
\caption{Training and validation accuracies for each baseline model. The average training accuracy is calculated across 5 seeds, each with four folds.}
...
To quantify this uncertainty, we measure inter-rater agreement using Krippendorff’s $\alpha$, obtaining a value of 0.6939. We additionally examine the consistency of agreement across annotators by performing a leave-one-out sensitivity analysis, producing $\alpha$ values ranging from 0.6602 to 0.7645 (Table~\ref{tab:rater_agreement}). These results indicate moderate variability in agreement across annotators, suggesting that annotator disagreement is not only driven by labeling mistakes, but also by subjective interpretations of severity. 

\begin{table}[h!]
\centering
\caption{Inter-rater agreement measured using Krippendorff's $\alpha$. The overall value is computed using labels from all four annotators. Minimum and maximum values correspond to a leave-one-annotator-out sensitivity analysis, highlighting the range of agreement across annotator subsets.}
\label{tab:rater_agreement}

\begin{tabular}{lc}
\toprule
\textbf{Metric} & \textbf{Value} \\
\midrule
Overall Krippendorff's $\alpha$ & 0.6939 \\
Minimum $\alpha$ & 0.6601 \\
Maximum $\alpha$ & 0.7645 \\
\bottomrule
\end{tabular}
\end{table}
