...
\caption{Training and validation accuracies for each baseline model. The average mean training accuracy is calculated across 5 seeds, each with four folds.}
...
To quantify this uncertainty, we measure inter-rater agreement using Krippendorff’s $\alpha$, obtaining a value of 0.6939. We additionally examine the consistency of agreement across annotators by performing a leave-one-annotator-out sensitivity analysis, producing $\alpha$ values ranging from 0.6602 to 0.7645 (Table~\ref{tab:rater_agreement}). These results indicate moderate variability in agreement across annotators, suggesting that annotator disagreement is not only driven by labeling mistakes, but also by subjective interpretations of severity. 
...
\begin{tabular}{lc}
\toprule
\textbf{Krippendorff's $\alpha$} & \textbf{Value} \\
\midrule
Overall & 0.6939 \\
Minimum & 0.6601 \\
Maximum & 0.7645 \\
