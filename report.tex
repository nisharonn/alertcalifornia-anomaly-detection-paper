...
After training each encoder, we obtain three sets of embeddings $\{\mathbf{e_T}, \mathbf{e_D}, \mathbf{e_H}\}$, by running inference on the labeled Coronado Hills data, $\mathbf{C_L}$. Since the classification head is trained with a supervised objective, we run inference only on the labeled dataset. 
