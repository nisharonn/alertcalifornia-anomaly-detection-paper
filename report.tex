...
For unseen cameras, classification accuracies reported in Table~\ref{tab:camera_accuracies} indicate an average accuracy of 77.76\%. However, accuracy alone does not fully capture model behavior, and in this setting, it hides a significant tradeoff between precision and recall. In particular, the model achieves a high overall recall of 0.9217 but a considerably lower precision of 0.4309, suggesting a tendency to over-predict \ac{QoS} anomalies at these new locations. This leads to more false positives despite stronger coverage of true anomalous events. 

Interestingly, Dew Drop and Toro Peak, both identified as seasonal cameras, outperform Berryessa, a location that does not observe significant environmental variability throughout seasons. With accuracies in the mid-80\% range compared to Berryessa’s 60.40\%, these results indicate that rather than seasonal factors, the model’s ability to generalize is more heavily influenced by other environmental characteristics. 
