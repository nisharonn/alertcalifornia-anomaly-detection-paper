...
Within each fold, we train a two-stage pipeline consisting of an encoder and a classification head. The encoder is trained on the training folds only, after which embeddings are computed for both training and validation sets. A classification head is then trained on the training embeddings and evaluated on the corresponding validation embeddings. Model performance is measured using classification accuracy, precision, and recall on both training and validation sets. After completing all cross-validation iterations, we compute the mean and \ac{SD} of metrics across the validation folds. 
...
Training accuracy & X & X & X & X \\
Test accuracy     & X & X & X & X \\
Test recall      & X & X & X & X \\
Test precision   & X & X & X & X \\
...
\caption{Classification results on unseen camera locations.}
...
The results demonstrate that embedding-based anomaly detection can effectively identify image quality degradations. In this section, we discuss the key takeaways from the final model performance and analyze the model’s ability to generalize across different camera locations. We also highlight the key limitations of our study and directions for future work. 

\subsection{Model Comparison and Selection}
Triplet loss demonstrates the strongest performance with an average validation accuracy of 0.9260 compared to DeepSAD’s 0.9094 and H-SAD’s 0.6903. Triplet loss is also more consistent, as accuracy varies much less across cross-validation folds and random seeds. The average standard deviation of validation accuracies is 0.0105 for triplet loss, compared to 0.0177 for DeepSAD and 0.0300 for H-SAD (Table~\ref{tab:train_val_accs}). The large gap between H-SAD and its alternatives likely reflects differences in label structure. Triplet loss and DeepSAD are trained using binary Normal/Abnormal labels, while H-SAD uses a custom 4-point severity scale. Learning more levels requires capturing more complex and nuanced ordinal distinctions in image quality degradation, which can make the classification task inherently more difficult. Based on these results, we adopt triplet loss for our final multi-camera model. 

\subsection{Generalization Across Camera Locations}
The final multi-camera model demonstrates ____ performance across all three camera locations in its training set. Table~\ref{tab:train_test_accs} shows ___ accuracies on ___, indicating the model (does/does not) generalizes to unseen environmental variations present in the training distribution. 
...
For unseen cameras, classification accuracies reported in Table~\ref{tab:camera_accuracies} indicate an average accuracy of X\%, 
