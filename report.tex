...
This trend becomes even more apparent when examining false positive frames from each unseen camera location, as seen in Figure~\ref{fig:false_positives}. At Toro Peak, the location most visually similar to those in the training set, false positives were exclusive to nighttime conditions. In low light settings, features in these frames likely became less distinguishable and deviated from those originally observed in the training cameras. Specifically, there is less visual information about terrain and landscape features, making it more difficult for the model to differentiate between natural environmental characteristics and true \ac{QoS} anomalies. Notably, under daytime conditions, all frames predicted as anomalous were accurately classified. A similar pattern emerges at Dew Drop, where 67.65\% of false positive frames were captured at night. Together, these observations suggest that reduced illumination introduces visual patterns that are less represented in the training data, in turn increasing the likelihood of false positives. 

Berryessa, however, presents a different challenge. Unlike all other cameras, Berryessa contains a large body of water, which is a visual feature unique to its location. Especially during nighttime conditions, this water surface resembles a layer of cloud (see Figure~\ref{fig:false_positives}). We hypothesize that this feature likely explains the disproportionately large number of false positives, as it mirrors visual cues that the model associates with \ac{QoS} anomalies. These findings further reinforce that the model is capable of generalizing across seasonal and environmental variation when the visual characteristics remain similar to those seen during training. However, performance declines when novel contexts are introduced. To achieve robust performance across the broader ALERTCalifornia network, a more diverse training set that better captures variation in terrain, illumination, seasonal conditions, and anomaly types is required. 
...
\begin{figure}[h!]
    ...
    \begin{subfigure}{0.31\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/toropeak_fp1.jpg}
        \caption{Toro Peak}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.31\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/dewdrop_fp1.jpg}
        \caption{Dew Drop}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.31\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/berryessa_fp2.jpg}
        \caption{Berryessa}
    \end{subfigure}
    ...
    \begin{subfigure}{0.31\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/toropeak_fp2.jpg}
        \caption{Toro Peak}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.31\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/dewdrop_fp2.jpg}
        \caption{Dew Drop}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.31\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/berryessa_fp1.jpg}
        \caption{Berryessa}
    \end{subfigure}

    \caption{False positive frames from each of the unseen camera locations.}
