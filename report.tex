...
We run inference only on the labeled data, since the classification head is trained with a supervised objective. We then examine each set of embeddings by using \ac{PCA} to reduce each $\mathbf{e_i} \in \{\mathbf{e_T}, \mathbf{e_D}, \mathbf{e_H}\}$ to ten dimensions. 
